...
my $deaths_file = 'raw_data/VSD349204_20231206_025927_97.csv';
my $pop_file    = 'raw_data/DPE403903_20231206_032556_74.csv';

load_deaths();
load_pop();

make_path('data') unless (-d 'data');
make_path('trends') unless (-d 'trends');
...
sub reformat_data {
	open my $out, '>:utf8', 'data/new_zealand_deaths_2000_2022.csv';
	...
	for my $year (2000 .. 2022) {
		for my $age_group (sort{$a <=> $b} keys %{$deaths{$year}}) {
			...
			my $population     = $pop{$year}->{$age_group}    // die "age_group : $age_group";
...
sub calc_trends_by_ages {
	for my $age_group (sort{$a <=> $b} keys %data) {
		...
		open my $out, '>:utf8', 'trends/' . $age_group_name. "_trends.csv";
