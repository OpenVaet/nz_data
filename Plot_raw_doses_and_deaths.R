...
data <- read.csv("deaths_and_doses_by_weeks.csv", header = TRUE)

# Find the maximum value for doses to scale the secondary y-axis accordingly
max_doses <- max(data$doses_administered)
max_deaths <- max(data$deaths)
scaling_factor <- max_deaths / max_doses

# Create a sequence of breaks every 20 weeks
breaks <- data$week_num[seq(1, nrow(data), by = 10)]
labels <- data$year_week[seq(1, nrow(data), by = 10)]
...
ggplot(data, aes(x = week_num)) + 
  ...
  geom_line(aes(y = doses_administered * scaling_factor, group = 2), colour = "red") +
  scale_y_continuous(
    "Deaths",
  ...
  labs(title = "Doses Administered and Deaths by Week Number") +
  theme_minimal() +
