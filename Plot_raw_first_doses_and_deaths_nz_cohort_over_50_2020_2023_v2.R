...
# Find the maximum value for nz_doses_administered to scale the secondary y-axis accordingly
max_nz_doses_administered <- max(data$nz_doses_administered)
max_deaths <- max(data$nz_deaths)
scaling_factor <- max_deaths / max_nz_doses_administered
...
ggplot(data, aes(x = week_num)) + 
  ...
  geom_line(aes(y = nz_doses_administered * scaling_factor, group = 2), colour = "red", size = 1.5) +
  scale_y_continuous(
    "NZ Deaths",
    labels = scales::comma,
    sec.axis = sec_axis(~./scaling_factor, name = "NZ Doses", labels = scales::comma)
  ) +
  scale_x_continuous(
    "Week Number",
    breaks = breaks,
  ...
  labs(title = "Deaths & Doses 1 in NZ by Week Number") +
  theme_minimal() +
  theme(
