...
open my $out, '>:utf8', 'data/deaths_and_pop_by_months_and_oia_age_groups.csv';
...
sub load_deaths {
	...
	while (<$in>) {
		...
		my $age_group = oia_age_group_from_age_groups_src($age_groups_src);
	...
	while (<$in>) {
		...
		if ($year eq ' ') {
			...
			for my $value_ref (1 .. scalar @values - 1) {
				...
				my $age_group = oia_age_from_age($header);
