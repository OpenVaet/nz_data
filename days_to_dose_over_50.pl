...
my $cutoff_date     = convert_compdate_to_date($cutoff_compdate);
my $nzwb_file       = 'data/nzwb_dob_removed.csv';
my $nz_file         = 'deaths_and_doses_by_weeks_2020_2023.csv';
...
sub load_nz_data {
	open my $in, '<:utf8', $nz_file;
	while (<$in>) {
		chomp $_;
		my ($year_week, $week_num, $doses_administered, $deaths) = split ',', $_;
		next if $year_week eq 'year_week';
		my ($year, $week_number) = split '-', $year_week;
		$nz_data{$year}->{$week_number}->{'week_num'} = $week_num;
		$nz_data{$year}->{$week_number}->{'nz_deaths'} = $deaths;
		$nz_data{$year}->{$week_number}->{'nz_doses_administered'} = $doses_administered;
	}
	close $in;
}
...
for my $mrn (sort{$a <=> $b} keys %nzwb_data) {
	...
	unless (exists $nzwb_data{$mrn}->{'doses'}->{'1'}) {
		...
		if ($joined_on <= $cutoff_compdate) {
			$dose_1++;
			my $date = Time::Piece->strptime($joined_on_dt, '%Y-%m-%d');
			my $week_number = $date->strftime('%U');
			my ($year) = split '-', $joined_on_dt;
			$week_number = '01' if $week_number eq '00';
			$nz_data{$year}->{$week_number}->{'cohort_doses_administered'}++;
			# die "date : $date" if $week_number eq '00';
			my ($days_in_study, $days_to_death);
			if ($nzwb_data{$mrn}->{'death_on_dt'} && ($nzwb_data{$mrn}->{'death_on'} <= $cutoff_compdate)) {
				my $date = Time::Piece->strptime($nzwb_data{$mrn}->{'death_on_dt'}, '%Y-%m-%d');
				my $week_number = $date->strftime('%U');
				my ($year) = split '-', $nzwb_data{$mrn}->{'death_on_dt'};
				$week_number = '01' if $week_number eq '00';
				$dose_1_died++;
				...
				$days_in_study = calculate_days_difference($joined_on_dt, $nzwb_data{$mrn}->{'death_on_dt'});
				$nz_data{$year}->{$week_number}->{'cohort_deaths'}++;
			} else {
				$days_in_study = calculate_days_difference($joined_on_dt, $cutoff_date);
			}
			$days_of_exposure{$days_in_study}++;
...
# Computing populations exposed by pool.
my %doe_by_pool = ();
for my $days_in_study (sort{$a <=> $b} keys %days_of_exposure) {
	my $total_subjects = $days_of_exposure{$days_in_study} // die;
	my ($days_to_death_group,
		$days_to_death_group_name) = group_time_to_death($days_in_study);
	$doe_by_pool{$days_to_death_group} += $total_subjects;
}
my %doe_summed_by_pool = ();
for my $days_to_death_group (sort{$a <=> $b} keys %doe_by_pool) {
	for my $days_to_death_group_over (sort{$a <=> $b} keys %doe_by_pool) {
		next unless $days_to_death_group_over >= $days_to_death_group;
		$doe_summed_by_pool{$days_to_death_group} += $doe_by_pool{$days_to_death_group_over}
	}
}
...
say $out_1 "days_to_death_group,days_to_death_group_name,total_deaths,deaths_per_10000000_doe";
for my $days_to_death_group (sort{$a <=> $b} keys %by_days_to_death_groups) {
	...
	my $doe_in_pool = $doe_summed_by_pool{$days_to_death_group} // die;
	my $deaths_per_10000000_doe = $total_deaths * 10000000 / $doe_in_pool;
	say $out_1 "$days_to_death_group,$days_to_death_group_name,$total_deaths,$deaths_per_10000000_doe";
}
close $out_1;
...
my $week_num = 0;
open my $out_3, '>:utf8', 'deaths_and_first_doses_by_weeks_2020_2023.csv';
say $out_3 "year_week,week_num,nz_doses_administered,nz_deaths,cohort_doses_administered,cohort_deaths";
for my $year (sort{$a <=> $b} keys %nz_data) {
	next if $year < 2021;
	for my $week_number (sort{$a <=> $b} keys %{$nz_data{$year}}) {
		$week_num++;
		my $cohort_doses_administered = $nz_data{$year}->{$week_number}->{'cohort_doses_administered'} // 0;
		my $cohort_deaths = $nz_data{$year}->{$week_number}->{'cohort_deaths'} // 0;
		my $nz_doses_administered = $nz_data{$year}->{$week_number}->{'nz_doses_administered'} // 0;
		my $nz_deaths = $nz_data{$year}->{$week_number}->{'nz_deaths'} // 0;
		say $out_3 "$year-$week_number,$week_num,$nz_doses_administered,$nz_deaths,$cohort_doses_administered,$cohort_deaths";
	}
}
close $out_3;
